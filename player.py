...
class Player:
    ...
    def __move_east(self):
        """
        Private method to move a generic player from their current position to one position east
        """
        self.__move(dx=1, dy=0)

    def __move_west(self):
        """
        Private method to move a generic player from their current position to one position west
        """
        self.__move(dx=-1, dy=0)

    def __move_north(self):
        """
        Private method to move a generic player from their current position to one position north
        """
        self.__move(dx=0, dy=-1)

    def __move_south(self):
        """
        Private method to move a generic player from their current position to one position south
        """
        self.__move(dx=0, dy=1)
